...
BOT_NAME = settings.NUBLADO_BOT["NAME"]
BOT_TOKEN = settings.NUBLADO_BOT["TOKEN"]
...
class NubladoBotConfig(AppConfig):
    ...
    def ready(self):
        from django_nublado_telegram.services.language import resolve_chat_language
        ...
        app.bot_data["repo_chat_id"] = settings.NUBLADO_BOT["REPO_ID"]
        register_handlers(app)

        bot = TelegramBot(
            name=BOT_NAME,
            application=app,
            webhook_url=settings.NUBLADO_BOT["WEBHOOK_URL"],
            webhook_token=settings.NUBLADO_BOT["WEBHOOK_SECRET"],
        )
